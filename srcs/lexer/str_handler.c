...
/*   Updated: 2024/12/20 16:56:42 by nlambert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * Compte le nombre de mots dans une chaîne de caractères en fonction des espaces.
 */
...
/**
 * Traite l'entrée lexer en mettant à jour l'état des guillemets et en comptant les mots.
 */
void process_lexer_input(char *str, int *i, int *j, t_quote *state)
{
	while (str[*i])
	{
		...
		if (!ft_white_space(str[*i]) && (!state->doubl_quot_status && !state->singl_quot_status))
		{
			(*j)++;
			(*i)++;
		}
		else if (ft_white_space(str[*i]) && (!state->doubl_quot_status && !state->singl_quot_status))
			break;
