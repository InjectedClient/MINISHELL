...
char *expand_token(const char *token, char **envp) {
    if (is_single_quote(token)) return handle_single_quote(token);
    if (is_double_quote(token)) return handle_double_quote(token, envp);
    if (starts_with_tilde(token)) return expand_tilde(token);
    if (starts_with_dollar(token)) return expand_variable(token, envp);
